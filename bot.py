...
CHAT_ID = "655537138"
REDIS_HOST = "climbing-narwhal-53855.upstash.io"
REDIS_PORT = 6379
REDIS_PASSWORD = "AdJfAAIjcDEzNDdhYTU4OGY1ZDc0ZWU3YmQzY2U0MTVkNThiNzU0OXAxMA"
PORT = 8080
TIMEFRAME = '15m'
MIN_BIG_BODY_PCT = 1.0  # Reverted to original 1.0%
MAX_SMALL_BODY_PCT = 1.0  # Reverted to original 1.0%
MIN_LOWER_WICK_PCT = 20.0  # Reverted to original 20.0%
MAX_WORKERS = 5  # Suitable for Replit
BATCH_DELAY = 3.0  # Increased slightly for stability
CAPITAL = 10.0
SL_PCT = 1.5 / 100
TP_SL_CHECK_INTERVAL = 30
CLOSED_TRADE_CSV = 'closed_trades.csv'
RSI_PERIOD = 14
ADX_PERIOD = 14
ZIGZAG_DEPTH = 12
...
# === PROXY ===
# No proxy credentials provided, so leaving as optional
proxies = {
    "http": os.getenv('PROXY_HTTP'),
    "https": os.getenv('PROXY_HTTPS')
} if os.getenv('PROXY_HTTP') and os.getenv('PROXY_HTTPS') else {}
...
def save_trades():
    if not redis_client:
        print("Redis client not initialized, cannot save trades")
        return
    try:
...
def load_trades():
    global open_trades
    if not redis_client:
        print("Redis client not initialized, cannot load trades")
        open_trades = {}
        return
    try:
...
def save_closed_trades(closed_trade):
    if not redis_client:
        print("Redis client not initialized, cannot save closed trades")
        return
    try:
...
def load_closed_trades():
    if not redis_client:
        print("Redis client not initialized, cannot load closed trades")
        return []
    try:
        ...
        if data:
            ...
            if len(trades) != len(unique_trades):
                print(f"Removed {len(trades) - len(unique_trades)} duplicate closed trades")
...
def send_telegram(msg, retries=3):
    ...
    for attempt in range(retries):
        try:
            response = requests.post(url, data=data, proxies=proxies if proxies else None, timeout=10).json()
            if response.get('ok'):
    ...
    print(f"Failed to send Telegram message after {retries} attempts: {msg[:50]}...")
    return None

def edit_telegram_message(message_id, new_text):
    if not message_id:
        print("Cannot edit Telegram message: Invalid message_id")
        return
...
def send_csv_to_telegram(filename):
    ...
    try:
        ...
        with open(filename, 'rb') as f:
            ...
            files = {'document': f}
            response = requests.post(url, data=data, files=files, proxies=proxies if proxies else None, timeout=15).json()
            if response.get('ok'):
...
exchange = ccxt.binance({
    'apiKey': os.getenv('BINANCE_API_KEY', ''),  # Add your Binance API key here or in Secrets
    'secret': os.getenv('BINANCE_API_SECRET', ''),  # Add your Binance API secret here or in Secrets
...
def detect_rising_three(candles):
    ...
    volume_decreasing = c1[5] >= c0[5]  # Relaxed condition
...
def export_to_csv():
    try:
        ...
        if not closed_trades_df.empty:
            exported_trades = redis_client.smembers('exported_trades') if redis_client else set()
...
def run_bot():
    try:
        if redis_client and redis_client.ping():
            ...
            print("❌ Redis connection failed or not initialized")
        load_trades()
